import { KEYWORDS, TOKENS } from "./constant";
import {
  isLetter,
  isDigit,
...
function getNum() {
  ...
  while (true) {
    switch (state) {
      ...
      case 9: //整数
        back();
        return new Token(TOKENS.NUMBER, word);
      default:
        console.log("a:非法元素");
    }
...
function getWord() {
  ...
  if (KEYWORDS.indexOf(word) != -1) {
    ...
    return new Token(TOKENS.ID, `symbos[${installID(word)}]:${word}`);
  }
}
/**
 * 获取位运算符或~关系运算符的token
...
function getBitOrRelationOperator() {
  ...
  while (c) {
    switch (state) {
      ...
      case 13:
      case 7: //位运算符
        return new Token(TOKENS.BIT_OPERATOR, stateMap[state]);
      case 2: //关系运算符
      case 4:
      case 6:
      case 8:
      ...
      case 12:
        //关系运算符
        return new Token(TOKENS.RELATION_OPERATOR, stateMap[state]);
    }
  }
}
/**
...
function getMathOperator() {
  ...
  if (c == getNext() && c == "+" && c == "-") {
    next();
    if (c == "+") return new Token(TOKENS.MATH_OPERATOR, "AUTO_INCREAMENT");
    else return new Token(TOKENS.MATH_OPERATOR, "AUTO_DECREMENT");
  } else {
    let map = { "+": "ADD", "-": "SUB", "*": "MUL", "/": "SUB", "%": "MOD" };
    return new Token(TOKENS.MATH_OPERATOR, map[c]);
  }
}
/**
 * 获取逻辑运算符token
...
function getLogicOperator() {
  ...
  while (c) {
    switch (state) {
      ...
      case 6:
        if (stateMap[state])
          return new Token(TOKENS.LOGIC_OPERATOR, stateMap[state]);
        break;
      case 7:
        return new Token(TOKENS.LOGIC_OPERATOR, c);
    }
  }
}
/**
  ...
  while (c) {
    switch (state) {
      case 0: //初始状态
        ...
        else if (isBitOrRelationOperator(c)) state = 10;
        else state = -1; //词法错误
        break;
      case 1: //数字
        token = getNum();
      ...
      case 2: //字母
        ...
        break;
      case 3: //分号
        return new Token(TOKENS.SECMICOLON, c);
      case 4: //逗号
        return new Token(TOKENS.COMMA, c);
      case 5: //三目运算符
        return new Token(TOKENS.TERNARY_OPERATORS, c);
      case 6: // 分隔符
        return new Token(TOKENS.DELIMITERS, c);
      case 7: // 算术运算符
      ...
      case 8: // 逻辑运算符
        ...
        break;
      case 9: // 赋值符
        token = new Token(TOKENS.ASSIGNMENT, "=");
        break;
      case 10: // 位运算符,关系运算符
