 ...
 */
function SemanticAnalyzer(s, products) {
      ...
      for (let i = 0; i < products.length; i++) {
        for (let j = i + 1; j < products.length; j++) {
          ...
          if (commonLeftFactor && n != 0) {
            ...
            // 添加消除公共左因子后的新产生式
            commonLeftFactor.unshift(newLeft);
...
function parse(tokens = []) {
  ...
  let leftNode = tree;
  products.map(p => {
    let nodes = p.right.map(x => {
      return new Tree(x);
    });
    leftNode.nodes = nodes;
    for (let i = 0; i < nodes.length; i++) {
      if (isNonTerminal(nodes[i].val)) {
        leftNode = nodes[i];
        break;
      }
    }
